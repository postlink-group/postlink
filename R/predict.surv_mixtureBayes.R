...
predict.surv_mixtureBayes <- function(object, newdata, 
  ...
  if (type == "response") {
    predictions <- switch(object$family,
                          ...
                          weibull = {
                            ...
                            exp(X_data %*% mean_coef) * gamma(1 + 1 / shape)  # AFT: E[T] = scale * Gamma(1 + 1/shape)
                          },
  ...
  if (type == "response") {
    all_preds <- switch(object$family,
                        ...
                        weibull = {
                          shape_draws <- object$estimates$shape  # vector of posterior shape samples
  ...
  # Credible intervals
  alpha <- 1 - level
  ci <- t(apply(all_preds, 1, quantile, probs = c(alpha / 2, 1 - alpha / 2)))
  lower <- ci[, 1]
  upper <- ci[, 2]
