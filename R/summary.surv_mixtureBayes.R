...
summary.surv_mixtureBayes <- function(object, ...) {
  ...
  ci1 <- t(apply(coef_draws1, 2, quantile, probs = c(0.025, 0.975)))
  ci2 <- t(apply(coef_draws2, 2, quantile, probs = c(0.025, 0.975)))
  TAB1 <- cbind(Estimate = apply(coef_draws1, 2, mean),
                `Std. Error` = apply(coef_draws1, 2, sd),
                `2.5 %` = ci1[,1],
                `97.5 %`= ci1[,2])
  TAB2 <- cbind(Estimate = apply(coef_draws2, 2, mean),
                `Std. Error` = apply(coef_draws2, 2, sd),
                `2.5 %` = ci2[,1],
                `97.5 %`= ci2[,2])
  # Prepare output list
  res <- list(call = object$call, family = object$family,
              coefficients = TAB1, m.coefficients = TAB2)
  # Family-specific parameters (shape/scale) summary
  if(object$family == "gamma") {
    # Use 'shape' for phi
    res$shape  <- mean(object$estimates$shape)    # mean of phi1 draws
    res$m.shape <- mean(object$estimates$m.shape) # mean of phi2 draws
  }
  if(object$family == "weibull") {
    ...
    # For convenience, store also as separate (to print one component at a time)
