...
generate_stan_surv <- function(components, priors = list()) {
  ...
  if (identical(components, c("gamma","gamma"))) {
    ...
    stan_code <- paste(
      ...
      "    real lp2;",
      "    if (event[n] == 1) {",
      "      lp1 = log(theta) + gamma_lpdf(time[n] | phi1, phi1 / exp(eta1[n]));",
      "      lp2 = log1m(theta) + gamma_lpdf(time[n] | phi2, phi2 / exp(eta2[n]));",
      "    } else {",
      "      lp1 = log(theta) + gamma_lccdf(time[n] | phi1, phi1 / exp(eta1[n]));",
      "      lp2 = log1m(theta) + gamma_lccdf(time[n] | phi2, phi2 / exp(eta2[n]));",
      "    }",
      "    z[n] = bernoulli_logit_rng(lp1 - lp2) + 1;",
      "  }",
      "}",
      sep = "\n"
    )
  ...
  if (identical(components, c("weibull","weibull"))) {
    ...
    stan_code <- paste(
      ...
      "    real lp2;",
      "    if (event[n] == 1) {",
      "      lp1 = log(theta) + weibull_lpdf(time[n] | shape1, scale1 * exp(eta1[n]));",
      "      lp2 = log1m(theta) + weibull_lpdf(time[n] | shape2, scale2 * exp(eta2[n]));",
      "    } else {",
      "      lp1 = log(theta) + weibull_lccdf(time[n] | shape1, scale1 * exp(eta1[n]));",
      "      lp2 = log1m(theta) + weibull_lccdf(time[n] | shape2, scale2 * exp(eta2[n]));",
      "    }",
      "    z[n] = bernoulli_logit_rng(lp1 - lp2) + 1;",
      "  }",
      "}",
