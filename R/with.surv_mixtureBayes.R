...
#'       \emph{Estimate}, \emph{Std. Error} (pooled), 95\% CI bounds (\emph{2.5%}, \emph{97.5%}), degrees of freedom (\emph{DF}), and fraction of missing information (\emph{FMI}).}
#'   \item{m}{Number of draws (imputations) used in pooling.}
#'
#' @details Estimates are pooled using Rubin's rules: the pooled estimate is the mean of draw-specific estimates, and the pooled variance is the sum of the within-imputation variance and the scaled between-imputation variance. 95\% confidence intervals use a t-distribution with the pooled degrees of freedom.  The fraction of missing information is computed as \((1+1/M)\,B/T\) for each parameter. 
#'
#' @examples
#' \dontrun{
