...
predict.glm_mixtureBayes <- function(object, newdata, 
  ...
  X_data <- model.matrix(as.formula(object$call$formula), data = newdata)
  ...
  if(type == "response"){
    predictions <- switch(object$family,
                          ...
                          binomial = plogis(X_data %*% mean_coef)) # logit link
  }
  ...
  if(type == "link"){
    # rows are new data points, columns are draws
    all_predictions <- switch(fit$family,
                       ...
                       binomial = plogis(X_data %*% t(object$estimates$coefficients))) # logit link
  } 
  
  se.predictions <- apply(all_predictions, 1, sd)
  ...
  alpha <- 1 - level
  ci <- t(apply(all_predictions, 1, quantile, probs = c(alpha/2, 1-alpha/2)))
